...
class Configuration:
    def __init__(self, unrestricted_times: WeeklyTimespans, rules: List[ProcessRule], enabled: bool):
    ...
    def from_json(cls, json_data: dict) -> 'Configuration':
        ...
        for rule_data in json_data.get("rules", []):
            ...
            rules.append(ProcessRule(
                process_regex=re.compile(proc_pat, re.IGNORECASE) if proc_pat else None,
                title_regex=re.compile(title_pat, re.IGNORECASE) if title_pat else None,
                action=action
            ))
